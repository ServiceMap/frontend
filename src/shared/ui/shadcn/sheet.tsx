...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      "tw: tw:fixed tw:inset-0 tw:z-50 tw:bg-black/80 tw:data-[state=closed]:animate-out tw:data-[state=closed]:fade-out-0 tw:data-[state=open]:animate-in tw:data-[state=open]:fade-in-0",
      className,
    )}
    {...props}
    ref={ref}
...
const sheetVariants = cva(
  "tw:fixed tw:z-50 tw:flex tw:flex-col tw:gap-4 tw:bg-background tw:p-6 tw:shadow-lg tw:transition tw:ease-in-out tw:data-[state=closed]:animate-out tw:data-[state=closed]:duration-300 tw:data-[state=open]:animate-in tw:data-[state=open]:duration-500",
  {
    variants: {
      side: {
        ...
        right:
          "tw:inset-y-0 tw:right-0 tw:h-full tw:w-3/4 tw:border-l tw:data-[state=closed]:slide-out-to-right tw:data-[state=open]:slide-in-from-right tw:sm:max-w-sm",
      },
    },
    defaultVariants: {
      side: "right",
  ...
  <SheetPortal>
    ...
    >
      <SheetPrimitive.Close className="tw:absolute tw:top-4 tw:right-4 tw:rounded-sm tw:opacity-70 tw:ring-offset-background tw:transition-opacity tw:hover:opacity-100 tw:focus:ring-2 tw:focus:ring-ring tw:focus:ring-offset-2 tw:focus:outline-none tw:disabled:pointer-events-none tw:data-[state=open]:bg-secondary">
        <X className="tw:h-4 tw:w-4 tw:cursor-pointer" />
